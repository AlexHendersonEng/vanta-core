...
TEST(VectorSum, ZerosPlusZeros) {
    ...
    auto c = run(a, b);
    for (int i = 0; i < n; ++i)
        EXPECT_FLOAT_EQ(c[i], 0.0f) << "index " << i;
}
...
TEST(VectorSum, OnesAndOnes) {
    ...
    auto c = run(a, b);
    for (int i = 0; i < n; ++i)
        EXPECT_FLOAT_EQ(c[i], 2.0f) << "index " << i;
}
...
TEST(VectorSum, NegativeValues) {
    ...
    auto c = run(a, b);
    for (int i = 0; i < n; ++i)
        EXPECT_NEAR(c[i], -2.0f, kEps) << "index " << i;
}
...
TEST(VectorSum, NonMultipleOfBlockSize) {
    ...
    for (int i = 0; i < n; ++i) { a[i] = static_cast<float>(i); b[i] = 1.0f; }
    auto c = run(a, b);
    for (int i = 0; i < n; ++i)
...
TEST(VectorSum, ExactlyOneBlock) {
    ...
    auto c = run(a, b);
    for (int i = 0; i < n; ++i)
        EXPECT_FLOAT_EQ(c[i], 5.0f) << "index " << i;
}
...
TEST(VectorSum, ExactlyTwoBlocks) {
    ...
    auto c = run(a, b);
    for (int i = 0; i < n; ++i)
        EXPECT_FLOAT_EQ(c[i], 3.0f) << "index " << i;
}
