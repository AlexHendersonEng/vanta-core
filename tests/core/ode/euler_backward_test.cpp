...
class EulerBackwardTest : public ::testing::Test {
 protected:
  // Allowed numerical tolerance for solution comparisons
  const double tolerance = 1e-6;
};

TEST_F(EulerBackwardTest, NegativeStepSize) {
  auto f = [](const double& t [[maybe_unused]], const std::vector<double>& y) {
    return std::vector<double>{y[0]};
  };

  EXPECT_THROW(
      { vanta::ode::EulerBackward(f, 0.0, 1.0, {1.0}, -0.1); },
      std::invalid_argument);
}

TEST_F(EulerBackwardTest, ZeroStepSize) {
  auto f = [](const double& t [[maybe_unused]], const std::vector<double>& y) {
    return std::vector<double>{y[0]};
  };

  EXPECT_THROW(
      { vanta::ode::EulerBackward(f, 0.0, 1.0, {1.0}, 0.0); },
      std::invalid_argument);
}

TEST_F(EulerBackwardTest, InvalidTimeInterval) {
  auto f = [](const double& t [[maybe_unused]], const std::vector<double>& y) {
    return std::vector<double>{y[0]};
  };

  EXPECT_THROW(
      { vanta::ode::EulerBackward(f, 1.0, 0.0, {1.0}, 0.1); },
      std::invalid_argument);
}

TEST_F(EulerBackwardTest, EqualStartEndTime) {
  auto f = [](const double& t [[maybe_unused]], const std::vector<double>& y) {
    return std::vector<double>{y[0]};
  };

  EXPECT_THROW(
      { vanta::ode::EulerBackward(f, 1.0, 1.0, {1.0}, 0.1); },
      std::invalid_argument);
}

TEST_F(EulerBackwardTest, ConstantFunction) {
  auto f = [](const double& t [[maybe_unused]], const std::vector<double>& y
              [[maybe_unused]]) { return std::vector<double>{0.0}; };

  vanta::ode::Solution sol = vanta::ode::EulerBackward(f, 0.0, 1.0, {5.0}, 0.1);
...
TEST_F(EulerBackwardTest, LinearFunction) {
  auto f = [](const double& t [[maybe_unused]], const std::vector<double>& y
  ...
  double t1 = 1.0;
  double h = 0.1;
  vanta::ode::Solution sol = vanta::ode::EulerBackward(f, t0, t1, {0.0}, h);

  // y(t) = t, so y(1.0) should be approximately 1.0
  EXPECT_NEAR(sol.y.back()[0], 1.0, tolerance);
}

TEST_F(EulerBackwardTest, ExponentialGrowth) {
  auto f = [](const double& t [[maybe_unused]], const std::vector<double>& y) {
    return std::vector<double>{y[0]};
  };
  ...
  double t1 = 1.0;
  double h = 0.01;
  vanta::ode::Solution sol = vanta::ode::EulerBackward(f, t0, t1, {1.0}, h);
...
TEST_F(EulerBackwardTest, MultiDimensionalSystem) {
  ...
  vanta::ode::Solution sol = vanta::ode::EulerBackward(f, t0, t1, y0, h);
...
TEST_F(EulerBackwardTest, CorrectNumberOfSteps) {
  auto f = [](const double& t [[maybe_unused]], const std::vector<double>& y
              [[maybe_unused]]) { return std::vector<double>{0.0}; };
...
TEST_F(EulerBackwardTest, TimeArrayCorrectness) {
  auto f = [](const double& t [[maybe_unused]], const std::vector<double>& y
              [[maybe_unused]]) { return std::vector<double>{0.0}; };
...
TEST_F(EulerBackwardTest, NonIntegerSteps) {
  auto f = [](const double& t [[maybe_unused]], const std::vector<double>& y
              [[maybe_unused]]) { return std::vector<double>{1.0}; };
...
TEST_F(EulerBackwardTest, TimeDependentFunction) {
...
TEST_F(EulerBackwardTest, LargeStepSize) {
  auto f = [](const double& t [[maybe_unused]], const std::vector<double>& y
              [[maybe_unused]]) { return std::vector<double>{1.0}; };
