...
TEST_F(RungeKutta4Test, NegativeStepSize) {
  ...
  EXPECT_THROW(
      { vanta::ode::RungeKutta4(f, 0.0, 1.0, {1.0}, -0.1); },
...
TEST_F(RungeKutta4Test, ZeroStepSize) {
  ...
  EXPECT_THROW(
      { vanta::ode::RungeKutta4(f, 0.0, 1.0, {1.0}, 0.0); },
      std::invalid_argument);
}
...
TEST_F(RungeKutta4Test, InvalidTimeInterval) {
  ...
  EXPECT_THROW(
      { vanta::ode::RungeKutta4(f, 1.0, 0.0, {1.0}, 0.1); },
      std::invalid_argument);
}
...
TEST_F(RungeKutta4Test, EqualStartEndTime) {
  ...
  EXPECT_THROW(
      { vanta::ode::RungeKutta4(f, 1.0, 1.0, {1.0}, 0.1); },
      std::invalid_argument);
}
...
TEST_F(RungeKutta4Test, ConstantFunction) {
  ...
  vanta::ode::Solution sol = vanta::ode::RungeKutta4(f, 0.0, 1.0, {5.0}, 0.1);
...
TEST_F(RungeKutta4Test, MultiDimensionalSystem) {
  ...
  vanta::ode::Solution sol = vanta::ode::RungeKutta4(f, t0, t1, y0, h);
